        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24">
          ...
          <div>
            <Card className="p-8 md:p-10 bg-card border-border shadow-xl">
              <div className="mb-8">
                <h2 className="text-2xl font-medium mb-2 text-foreground">Send a Message</h2>
                <p className="text-sm text-foreground-soft">Fill out the form below and we'll get back to you.</p>
              </div>

              <form className="space-y-6" onSubmit={onSubmit}>
                <div className="space-y-2.5">
                  <label className="text-sm font-medium text-foreground-soft">I am a...</label>
                  <select className="w-full bg-background border border-border/80 rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary/50 transition-all text-foreground">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div className="space-y-2.5">
                    <label className="text-sm font-medium text-foreground-soft">Your Name</label>
                    <input
                      value={name}
                  ...
                  <div className="space-y-2.5">
                    <label className="text-sm font-medium text-foreground-soft">Your Email</label>
                    <input
                      value={email}
                ...
                <div className="space-y-2.5">
                  <label className="text-sm font-medium text-foreground-soft">Subject</label>
                  <input
                    value={subject}
                ...
                <div className="space-y-2.5">
                  <label className="text-sm font-medium text-foreground-soft">Message</label>
                  <textarea
                    value={message}
                    onChange={(e) => setMessage(e.target.value)}
                    placeholder="Provide details about your inquiry..."
                    className="w-full bg-background border border-border/80 rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary/50 transition-all placeholder:text-foreground-muted min-h-[120px] resize-y"
                  />
                </div>

                <div className="space-y-2.5">
                  <label className="text-sm font-medium text-foreground-soft">Upload CV or Job Brief (Optional)</label>
                  <div
                    onDragOver={(e) => e.preventDefault()}
                    onDrop={onDrop}
                    className="w-full bg-background border border-dashed border-border/80 hover:border-primary/50 rounded-lg p-8 text-center text-sm transition-all group"
                  >
                    <input
                      ref={fileRef}
                      type="file"
                    ...
                    />
                    <UploadCloud className="mx-auto mb-3 h-8 w-8 text-foreground-muted group-hover:text-primary transition-colors" />
                    <div className="text-foreground mb-1">
                      Drag & drop file or{" "}
                      <button
                        type="button"
                        onClick={() => fileRef.current?.click()}
                        className="text-primary font-medium hover:underline outline-none focus-visible:ring-2 focus-visible:ring-primary rounded-sm"
                      >
                        browse
                      </button>
                    </div>
                    ...
                    {file && (
                      <div className="mt-4 inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-primary/10 text-xs font-medium text-primary border border-primary/20">
                        {file.name}
                      </div>
                    )}
                    {error && (
                      <div className="mt-3 text-xs text-destructive bg-destructive/10 border border-destructive/20 rounded px-3 py-2">
                        {error}
                      </div>
                    )}
                  </div>
                ...
                <div className="flex items-center gap-3 pt-2">
                  ...
                  <label
                    htmlFor="agree"
                    className="text-sm text-foreground-soft cursor-pointer select-none"
                  >
                ...
                <div className="pt-4">
                  <Button
                    type="submit"
                    className="w-full bg-primary/20 hover:bg-primary/30 text-primary border border-primary/30 h-[52px] text-base font-medium rounded-xl transition-all"
                  >
                    Send Message
                  </Button>
                </div>
