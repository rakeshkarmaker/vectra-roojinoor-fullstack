      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        <div className="flex flex-wrap justify-around items-center gap-8 md:gap-12">
          {industries.map((item, index) => (
            ...
            >
              <div className=" text-accent group-hover:text-accent-foreground group-hover:scale-110  transition-all duration-300">
                {item.icon}
              </div>
              <span className="text-sm font-medium text-center whitespace-nowrap text-accent group-hover:text-accent-foreground transition-colors duration-300">
                {item.label}
              </span>
            </motion.div>
          ))}
